...
### Halo data
colnames = ['halo_id','halo_upid','halo_pid','halo_hostid','halo_x','halo_y',\
            'halo_z','halo_vx','halo_vy','halo_vz','halo_mvir','halo_rvir',\
            'halo_rs','halo_vpeak','CS_flag']

halo_prop_table = pd.read_csv(path_to_interim + 'halo_catalog.csv',\
                                header=None,delimiter='\t',names=colnames)
v_sim = 130**3 #(Mpc/h)^3
vpeak = halo_prop_table.halo_vpeak.values
...
#        print('More than one match found', end='\r')
...
mock_catalog = halo_prop_table.merge(Mr_vpeak_catalog,how='inner',\
                           left_on=halo_prop_table.index.values,\
                           right_on=Mr_vpeak_catalog.index.values,\
                           right_index=True)
mock_catalog = mock_catalog.drop(['key_0'],axis=1)
mock_catalog.columns = colnames + ['v_peak(km/s)','M_r','logMbary(logMsun)',\
                                   'Re(kpc)']
